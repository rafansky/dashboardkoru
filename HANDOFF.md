...
- Correccion posterior de avatares: las fotos de VPG se sirven mediante `GET /api/tactical-avatar`, limitado al CDN oficial de VPG, para evitar los bloqueos CORS de WebGL. El 3D usa una capa de textura propia para el retrato y al abrir una pizarra sincroniza las entidades antiguas sin foto con la plantilla actual por `rosterKey` o nombre.
